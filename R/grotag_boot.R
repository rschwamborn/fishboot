...
#' library(fishmethods)
#' data(bonito)
#' res <- grotag_boot(bonito, nresamp = 10, L1=L1, L2=L2, T1=T1, T2=T2,alpha=35,beta=55, design=list(nu=1,m=1,p=1,sea=1), stvalue=list(sigma=0.9,nu=0.4,m=-1,p=0,u=0.4,w=0.4), upper=list(sigma=5,nu=1,m=2,p=0.5,u=1,w=1), lower=list(sigma=0,nu=0,m=-2,p=0,u=0,w=0),control=list(maxit=1e4))
#' res$bootRaw
#' res$seed
#' library(fishboot)
#' LinfK_scatterhist(res)
#' @export
grotag_boot <- function(input.data, nresamp = 200, L1 = NULL, L2 = NULL, T1 = NULL, T2 = NULL, alpha = NULL, beta = NULL, design = list(nu = 0, m = 0, p = 0, sea = 0), stvalue = list(sigma = 0.9, nu = 0.4, m = -1, p = 0.01, u = 0.4, w= 0.4), upper = list(sigma= 5, nu = 1, m = 2, p = 1, u = 1, w = 1), lower = list(sigma = 0, nu = 0, m = -2, p = 0, u = 0, w = 0), gestimate = TRUE, st.ga = NULL, st.gb = NULL, st.galow = NULL, st.gaup = NULL, st.gblow = NULL, st.gbup = NULL, control = list(maxit = 10000)){
  res_tab <- data.frame(rep.int(NA,(nresamp)), rep.int(NA,(nresamp)),
                        rep.int(NA,(nresamp)),
                        rep.int(NA,(nresamp)),
                        rep.int(NA,(nresamp)),
                        rep.int(NA,(nresamp)),
